...
exports.createOdooSession = exports.logoutRedirect = exports.verify = exports.login = void 0;
...
const loginSchema = zod_1.z.object({
    ...
    isRegistering: zod_1.z.boolean().optional().default(false)
});
...
const registrationCache = new node_cache_1.default({ stdTTL: 3600 });
// Session cache for authenticated users (expires in 24 hours)
const sessionCache = new node_cache_1.default({ stdTTL: 86400 });
...
const login = async (req, res) => {
    ...
    try {
        ...
        if (isRegistering) {
            const existingUser = await odoo_service_1.OdooService.findUserByLogin(email);
            if (existingUser) {
            ...
            await odoo_service_1.OdooService.sendPreRegistrationEmail(email, token);
            res.json({
                success: true,
        ...
        let user = await odoo_service_1.OdooService.userAuthenticate(email, password);
        if (!user) {
...
const verify = async (req, res) => {
    ...
    try {
        const cachedData = registrationCache.get(token);
        if (cachedData && cachedData.email === email) {
            ...
            if (!existingUser) {
                ...
                await odoo_service_1.OdooService.createUser(cachedData.email, cachedData.password, cachedData.name);
            }
            else {
                console.log(`[AUTH] User already exists in Odoo, skipping creation: ${email}`);
            }
            // Remove from cache
            registrationCache.del(token);
            const response = {
                success: true,
        ...
        const success = await odoo_service_1.OdooService.verifyUser(email, token);
        if (success) {
...
const logoutRedirect = async (req, res) => {
    try {
...
const createOdooSession = async (req, res) => {
    try {
        // Validate input
...
exports.createOdooSession = createOdooSession;
