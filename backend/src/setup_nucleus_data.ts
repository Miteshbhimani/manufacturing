...
async function setupNucleusData() {
  try {
    ...
    try {
      ...
      const demoProds = await OdooService.executeKw('product.template', 'search', [[['name', 'ilike', 'Nexus'], '|', ['name', 'ilike', 'V5-Axis'], ['name', 'ilike', 'H-Series']]]);
      if (demoProds.length > 0) {
        await OdooService.executeKw('product.template', 'unlink', [demoProds]);
    ...
    try {
      ...
      for (const p of NUCLEUS_PRODUCTS) {
        const exists = await OdooService.executeKw('product.template', 'search', [[['name', '=', p.name]]]);
        let productId;
        const { cat, img, has3D, filename, ...pData } = p;
        
        if (exists.length > 0) {
          productId = exists[0];
          await OdooService.executeKw('product.template', 'write', [[productId], pData]);
        } else {
          productId = await OdooService.executeKw('product.template', 'create', [pData]);
        }
        
        const categories = await OdooService.executeKw('product.category', 'search', [[['name', '=', cat]]]);
        let catId = categories.length > 0 ? categories[0] : await OdooService.executeKw('product.category', 'create', [{ name: cat }]);
        await OdooService.executeKw('product.template', 'write', [[productId], { categ_id: catId }]);
        ...
        if (img && filename) {
          try {
            ...
            await OdooService.executeKw('product.template', 'write', [[productId], { image_1920: base64 }]);
          } catch (e) {
