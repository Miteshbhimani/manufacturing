...
import { ApiResponse, OdooProduct, ProcessedProduct } from '../types/express.types';
import { z } from 'zod';
...
const seedCategoriesSchema = z.array(categorySchema);

export const getAllProducts = async (req: Request, res: Response): Promise<void> => {
    try {
        const isDevelopment = process.env.NODE_ENV !== 'production';
        
        const CORE_PRODUCTS: ProcessedProduct[] = [
          {
            id: 1,
          ...
          },
          {
            ...
            list_price: 2800,
            description: "High-performance impeller for industrial pumps. Features: SS 316 material, precision engineering, high efficiency.",
            description_sale: "Precision impeller in SS 316 for high-performance applications.",
            categ_id: [2, "Pump Components"],
...
export const getProductById = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const product = CORE_PRODUCTS.find(p => p.id === parseInt(id));
...
export const getProducts = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const odooProducts = await OdooService.executeKw(
        'product.template',
        'search_read',
        [[['sale_ok', '=', true]]],
        { fields: ['id', 'name', 'list_price', 'description_sale', 'categ_id', 'description', 'image_1920'], limit: 50 }
      ) as OdooProduct[];
      ...
      if (odooProducts.length === 0) {
        ...
        const response: ApiResponse<ProcessedProduct[]> & { isFallback?: boolean } = { success: true, data: CORE_PRODUCTS, isFallback: true };
        res.json(response);
        return;
      }
      
      const odooUrl = process.env.ODOO_URL!;
      const enriched = odooProducts.map((p: OdooProduct) => {
        if (isDevelopment) {
      ...
      const response: ApiResponse<ProcessedProduct[]> = { success: true, data: enriched };
      res.json(response);
    } catch (error: any) {
      console.error("Odoo product fetch failed, returning core catalog:", error.message);
      const fallbackResponse: ApiResponse<ProcessedProduct[]> & { isFallback?: boolean } = { success: true, data: CORE_PRODUCTS, isFallback: true };
      res.json(fallbackResponse);
    }
};

export const syncEncoreProducts = async (req: Request, res: Response): Promise<void> => {
    const urls = [
...
export const getCategories = async (req: Request, res: Response): Promise<void> => {
    try {
      const odooCategories = await OdooService.executeKw(
        'product.category',
        'search_read',
        [[]],
        { fields: ['id', 'name'] }
...
export const seedCategories = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        for (const cat of cats) {
            const exists = await OdooService.executeKw('product.category', 'search', [[['name', '=', cat]]]);
            if (exists.length === 0) {
                const id = await OdooService.executeKw('product.category', 'create', [{ name: cat }]);
                created.push({ id, cat });
            }
        }
