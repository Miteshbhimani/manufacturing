...
export const createLead = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const leadId = await OdooService.executeKw(
      'crm.lead',
      'create',
      [{
    ...
    try {
      const channels = await OdooService.executeKw(
        'discuss.channel', 
        'search_read', 
        [[['name', 'ilike', 'general']]], 
        { fields: ['id'], limit: 1 }
      ...
      const bots = await OdooService.executeKw('res.partner', 'search', [[['name', 'ilike', 'odoobot']]]);
      ...
      if (channels && channels.length > 0) {
        ...
        await OdooService.executeKw('discuss.channel', 'message_post', [channels[0].id], {
          body: messageBody,
